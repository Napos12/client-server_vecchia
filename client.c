...
void read_mode()
{
    ...
    if(check_directory(cl.remote_path) == 1 && check_directory(path) == 1){
      cl.local_file_n = strdup(create_file(cl.remote_file_n, cl.remote_path));      // Nel caso in cui il percorso esiste controlla se esiste il file e nel caso ne crea un altro con lo stesso nome
    }
    else{
...
int main(int argc,char* argv[])
{
    ...
    recv(client_fd,NULL,BUFFER_SIZE,0);

    send(client_fd,"fanculo",BUFFER_SIZE,0);
    if(cl.read==1)
    {
